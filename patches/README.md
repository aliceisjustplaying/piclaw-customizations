...
| 31 | `api.ts`, thread-switch web UI lifecycle/orchestration files, refresh coordination helpers, related web tests | Consolidated UI perf tracing for thread switching and branch creation, plus switch-aware refresh coalescing to cut duplicate hydration/status fetches while preserving the `window.__PICLAW_PERF__` runtime contract |
